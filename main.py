...
class BasePlane(object):
    ...
    def checkBullet(self, enemy, minRange):
        hit = 0
        for bullet in enemy.bulletList:
            if(bullet.x + minRange > self.x and bullet.x - minRange < self.x and bullet.y + minRange > self.y and bullet.y - minRange < self.y):
                enemy.bulletList.remove(bullet)
                hit += 1
        return hit
...
def main():
    ...
    while True:
        ...
        # 检测碰撞
        checkHero = hero.checkBullet(enemyplane, 2)
        if checkHero > 0:
            print('hero was shooted')
            hero.Hp -= checkHero
            if(hero.Hp  == 0):
                print('you lose')
                break

        checkEnermy = enemyplane.checkBullet(hero, 8)
        if checkEnermy > 0:
            print("enermy was shooted")
            enemyplane.Hp -= checkEnermy
            if(enemyplane.Hp == 0):
                print("you win")
                break
        # 显示玩家飞机的图片
